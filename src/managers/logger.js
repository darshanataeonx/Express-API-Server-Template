...
class Logger extends console.Console {
        ...
        static logRequest(request) {
                ...
                instance.info(
                        request["x-request-id"],
                        `Method = ${request.method}, Query = ${JSON.stringify(request.query)}, Params = ${JSON.stringify(request.params)}, Body = ${JSON.stringify(request.body)}, Files = ${request.files ? "true" : "false"}`,
                );
        }
}
