...
class RouteManager {
    ...
    async #setEssentialRoutes() {
        ...
        this.#expressServerInstance.use(require('express-status-monitor')())
        this.#expressServerInstance.use(async (request, response, next) => {
            ...
            response.on('finish', async () => {
                ...
                if (request['__databaseConnection']) {
                    await request['__databaseConnection'].release();
                }
            });
            next();
        });
    ...
    async registerCustomRoutes() {
        const routers = this.readRoutes(__dirname + '/../routes/');
        Object.keys(routers).forEach(basePath => {
            this.#expressServerInstance.use(basePath, routers[basePath]);
        });
    ...
    async #setErrorHandlingRoute() {
        this.#expressServerInstance.use(async (err, req, res, next) => {
            ...
            res.status(500).json({
                error: true,
                message: getConfigValue('') === "live" || getConfigValue('') === "production" ? 'Internal Server Error' : err.message
            });
        });
    ...
    readRoutes(directory) {
        const fs = require('fs');
        if (!fs.existsSync(directory)) throw new Error(`Invalid routes directory '{${directory}}'.`);
        const routers = {};
