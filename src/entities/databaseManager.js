...
class DatabaseManager {
    ...
    async acquireConnection(requestId) {
        ...
        this.#logger.info(requestId, 'Acquiring connection to the database server...');
        try{
            const connection = await this.#pool.getConnection();
            this.#logger.info(requestId, 'Connection to the database sever created successfully.');
            return connection;
        } catch(error){
            this.#logger.error(requestId, `Can\'t connect to the database server. Error: ${error.message}`);
        }
    }
    ...
    async _executeQuery(sql, placeholderValues) {
        try{
            ...
            this.#logger.error('SYS', `Unable to execute sql query. Query: ${sql} | Error: ${error.message}.`);
        }
    }
    ...
    async verifyConnection(){
        if(!this.connection) throw new Error("Can not verify the empty database connection.");
        ...
        try{
            ...
            this.#logger.error('SYS', `Database server connection verification failed. Error: ${error.message}.`);
        }
    }
}
